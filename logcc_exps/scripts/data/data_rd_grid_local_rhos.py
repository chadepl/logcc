""" This file runs the different CC methods on the three real datasets we have.
It runs CC with different correlation thresholds and stores the clusterings and times.
"""
# Standard library
...
from logcc_exps.scripts.data.data_rd_grid import hptc_3d
...
from logcc_exps.lib.clustering.pfaffelmoser2012 import (
    correlation_clustering as pffaffelmoser2012,
)
...
if __name__ == "__main__":
    ...
    data_dir = DATA_DIR / "rd_grid_local_rhos" # NOTE: if running 3d datasets (by commenting the rest): change path to rd_grid_local_rhos_3d and comment "pfaffelmoser 2012".
    ...
    DATASETS = [
            ...
            # ("hptc_right_parotid_3d", hptc_3d, dict(ana_struct="Parotid_R", only_ensemble=True, iso_value=None, range_num_slices=None)), # run only with pivot
            # ("hptc_brainstem_3d", hptc_3d, dict(ana_struct="BrainStem", only_ensemble=True, iso_value=None, range_num_slices=None)), # run only with pivot
        ] 
    ...
    for dataset_name, dataset_fn, dataset_kwargs in DATASETS:
        ...
        for i, t in enumerate(iterables):
            ...
            seed, method, local_rho, rho = t

            fn_base = [str(a) for a in (method, ACCELERATE_FLAG, local_rho, rho, seed)]

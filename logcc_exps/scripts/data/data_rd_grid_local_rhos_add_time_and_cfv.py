...
if __name__ == "__main__":
    ...
    data_dir = DATA_DIR / "data/rd_grid_local_rhos/"
    ...
    for d in dataset_path.glob("*.pkl"):
        with open(d, "rb") as f:
            ...
            if record["method"] == METHOD:
                print(record["dataset"], record["method"], record["local_rho"], record["rho"], record["accelerate_flag"], record["t_local"], record["t_global"], record["t_total"])

                clustering_records.append(record)
                
    df_times = table_times_data(ensemble, clustering_records, data_dir, f"{dataset_path.stem}-{METHOD}-times")
    df_cfv = table_cost_data(ensemble, clustering_records, data_dir, f"{dataset_path.stem}-{METHOD}-cfv")
